...
```json
{
  ...
  "tier": 2,
  "confidence": "advisory",
  "event_identity": "fr_doc:2026-12345",
...
| `source_kind` | string | The `SourceKind` tag name (`model_list_probe`, `statement_watch`, `federal_register`, `federal_register_public_inspection`, `feed_watch`, `keyword_watch`, `market_watch`). |
...
| `event_identity` | string | Normalized identity used to coalesce corroborating sources (e.g. `model_present:claude-fable-5`, `statement_restored`, `fr_doc:<number>`). |
